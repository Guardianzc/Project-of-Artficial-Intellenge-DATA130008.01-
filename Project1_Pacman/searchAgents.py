    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                explored = explore
                if ((nextplace not in explore) and (nextplace in self.corners)):
                    explored = explore + [nextplace]
                successors.append([[nextplace, explored] , action, 1])
        self._expanded += 1 # DO NOT CHANGE
        return successors
...
def cornersHeuristic(state, problem):
    ...
    Heuristic = DistanceClosestCorner(state[0], cornerlist)
    return Heuristic # Default to trivial solution

def DistanceClosestCorner(start, cornerlist):
    totaldis = 0
    minj = 0
    if not cornerlist:
        return 0
    ...
    for j in range(len(cornerlist)):
        distance = abs(cornerlist[j][0] - start[0]) + abs(cornerlist[j][1] - start[1])
        if distance < mintotal:
            mintotal = distance
            minj = j
    start = cornerlist[minj] 
    del(cornerlist[minj])
    totaldis = mintotal + DistanceClosestCorner(start, cornerlist)
    return totaldis
...
def foodHeuristic(state, problem):
    ...
    for loc in foodloc:
        ...
        singleprob = PositionSearchProblem(Gamestate, start = position, goal = loc,warn = False,visualize = False)
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        for food in foodList:
            problem = PositionSearchProblem(gameState, start = startPosition, goal = food, warn = False, visualize=False)
