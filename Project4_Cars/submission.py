...
class ExactInference(object):
    ...
    def elapseTime(self):
        ...
        for Row in range(self.belief.getNumRows()):
            for Col in range(self.belief.getNumCols()):
                Old_Prob[(Row, Col)] = self.belief.getProb(Row, Col)
                self.belief.setProb(Row, Col, 0) 
                # Without this all the prob of state will be the same?
                # Because we use addProb, cause we need to set the initalize value to 0
        for key, value in self.transProb.items():
            (oldTile, newTile) = key
            Post_Pri = Old_Prob[oldTile] * value
            self.belief.addProb(newTile[0], newTile[1], Post_Pri)
...
class ParticleFilter(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for particle in self.particles:
            Pri_prob = self.particles[particle]
            (Row, Col) = particle
            X, Y = util.colToX(Col), util.rowToY(Row)
            Distance = math.sqrt((X - agentX) ** 2 + (Y - agentY) ** 2)
            Likelihood = util.pdf(observedDist, Const.SONAR_STD, Distance)
            NewProb[particle] = Pri_prob * Likelihood
        #Resample
        self.particles = collections.defaultdict(int)
        for i in range(self.NUM_PARTICLES):
            self.particles[util.weightedRandomChoice(NewProb)] += 1
